<template>
  <Card
    :title="signup ? $t('Sign up') : $t('Authorization Required')"
  ...
  >
    ...
    <p v-if="signup" style="text-align: center">
      {{ $t('Already have an account?')
      }}<a href="/login" style="text-decoration: none; color: #1c7cd6">{{
        $t('Sign in')
      }}</a>
    </p>
    <p v-else style="text-align: center">
      {{ $t('New to Rttys?')
        ...
        href="/login?signup=1"
        style="text-decoration: none; color: #1c7cd6"
        >{{ $t('Sign up') }}</a
      >
    </p>
  </Card>
        ...
        if (valid) {
          ...
          if (this.signup) {
            ...
            this.axios
              ...
              .catch(() => {
                this.$Message.error(
                  this.$t('Signin Fail! username or password wrong.').toString()
                );
              });
          }
        }
